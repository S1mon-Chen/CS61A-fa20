    ...
    def nearest_bee(self, beehive):
        ...
        while place != beehive:
            if self.min_range <= distance <= self.max_range:
                has_bee = rANTdom_else_none(place.bees)
                if has_bee:
                    return has_bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount < self.armor:
            ...
            total_demage = self.armor + self.damage
        for bee in self.place.bees[:]:
            bee.reduce_armor(total_demage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        has_bee = rANTdom_else_none(self.place.bees)
        if self.digesting > 0:
            self.digesting -= 1
        elif has_bee:
            self.eat_bee(has_bee)
        # END Problem 6
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        ScubaThrower.__init__(ScubaThrower, armor)
    ...
    def action(self, gamestate):
        ...
        if not self.is_true_queen:
            ...
            while place != None:
                if place.ant:
                    ant = place.ant
                    if ant.can_double and not ant.doubled:
                        ant.damage *= 2
                        ant.doubled = True
                place = place.exit
        # END Problem EC
...
def make_slow(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def new_action(slow_bee, gamestate):
...
def make_scare(action, bee):
    ...
    # BEGIN Problem Optional 4
    "*** YOUR CODE HERE ***"
    def new_action(scared_bee, gamestate):
        bee.direction = False
        action(gamestate)
        bee.direction = True
    return new_action
...
def apply_status(status, bee, length):
    ...
    def action(bee, gamestate):
        nonlocal length
        if length > 0:
            length -= 1
            return new_action(bee, gamestate)
        else:
            return original_action(gamestate)
    bee.action = lambda gamestate: action(bee, gamestate)
    # END Problem Optional 4
