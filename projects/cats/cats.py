...
def accuracy(typed, reference):
    ...
    for typed_word in typed_words:
        if i >= len(reference_words):
            denom += len(typed_words) - len(reference_words)
...
def report_progress(typed, prompt, user_id, send):
    ...
    for typed_word in typed:
        if typed_word == prompt[i]:
            numer += 1
        else:
            break
        i += 1
    progress = numer / len(prompt)
...
def time_per_word(times_per_player, words):
    ...
    for timestamps in times_per_player:
        ...
        for i in range (1, len(timestamps)):
            time = time + [timestamps[i] - timestamps[i - 1]]
        times = times + [time]
...
def fastest_words(game):
    ...
    for word_index in word_indices:
        time = all_times(game)[0][word_index]
        for player_index in player_indices:
            time = min(time, all_times(game)[player_index][word_index])
        for player_index in player_indices:
            if all_times(game)[player_index][word_index] == time:
                fastest_list[player_index] += [word_at(game, word_index)]
                break
    return fastest_list
    # END PROBLEM 10
