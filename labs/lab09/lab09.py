...
def subseqs(s):
    ...
    if not s:
        ...
        no = subseqs(s[1:])
        return insert_into_all(s[0], subseqs(s[1:])) + no
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    """
    # >>> num_trees(4)
    # 5     (num_trees(1) * num_trees(3)) * 2 + (num_trees(2) * num_trees(2))
    # >>> num_trees(5)
    # 14     (num_trees(1) * num_trees(4)) * 2 + (num_trees(2) * num_trees(3)) * 2
    if n == 1:
        return 1
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        for info in typing_input:
            output, self.buttons[info].times_pressed = output + self.buttons[info].key, self.buttons[info].times_pressed + 1
        return output
