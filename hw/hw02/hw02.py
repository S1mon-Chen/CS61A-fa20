...
def count_coins(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(total_value, coin_value):
        if total_value < coin_value:
            return 0
        if total_value == coin_value:
            return 1
        if coin_value < 25:
            return helper(total_value, next_largest_coin(coin_value)) \
                 + helper(total_value - coin_value, coin_value)
        else:
            return helper(total_value - coin_value, coin_value)
