...
def balanced(m):
    ...
    if is_mobile(m):
        ...
        if balanced(end(left(m))) and balanced(end(right(m))) and left_torque == right_torque:
            return True
        else:
            return False
    if is_planet(m):
...
def preorder(t):
    ...
    if not is_leaf(t):
        preorder_list += sum((preorder(branch) for branch in branches(t)), [])
...
def quadratic(x, a, b, c):
    ...
    upper = upper_bound(x)
    X_point = - b / (2 * a)
    left_val = a * lower * lower + b * lower + c
    right_val = a * upper * upper + b * upper + c
    X_val = a * X_point * X_point + b * X_point + c
    if X_point > lower and X_point < upper:
        return interval(min(left_val, right_val, X_val), max(left_val, right_val, X_val))
    else:
